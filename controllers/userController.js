    ...
    try {
        ...
        if (!u) {
            ...
            const mailOptions = {
                from: `${process.env.EMAIL_ADDRESS} `,
                to: `${email} `,
                subject: "Code: For Change Password",
                text:
                    `Dear ${name} \, \n\n` +
                    "Code for Change Password is : \n\n" +
                    `${otp} \n\n` +
...
const verifyOTP = async (req, res) => {
    if (!req.body.number || !req.body.email) {
        return res.json({ type: "failure", result: "either email or Code is undefined" })
    }
    ...
    if (now > new Date(data.expireTime)) {
        return res.status(401).json({ type: "failure", result: "Code has been expired" });
    } else {
        if (otp == data.otp) {
            res
                .status(200)
                .json({ type: "success", result: "Code has been verified" });
        } else {
            res.status(401).json({ type: "failure", result: "Code is incorrect" });
        }
    }
};

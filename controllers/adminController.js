  ...
  try {
    ...
    const users = await User.find({
      $and: [
        ...
        { email: { $ne: 'systemdesign202305@gmail.com' } },
      ],
    }).sort({ $natural: -1 });

    if (!user) return res.json({ type: 'failure', result: 'No user found' });
...
exports.DeletePostByAdmin = async (req, res, next) => {
  try {
    ...
    const users = await Post.find({}).sort({ $natural: -1 }).populate('Author');
    if (!users) return res.json({ type: 'failure', result: 'No user found' });
    // const { password, ...rest } = update
    res.json({
      type: 'success',
      result: `Account has been deleted `,
...
exports.GetAllPosts = async (req, res, next) => {
  try {
    const user = await Post.find({}).sort({ $natural: -1 }).populate('Author');
    if (!user) {
      return res.json({ type: 'failure', result: 'No user found' });
    }
    res.json({ type: 'success', result: user });
    ...
    if (username && email && message) {
      ...
      if (contact.save())
        res.status(200).json({
          type: 'success',
          result:
            'Message Saved Successfully. we will get back to you as soon as possible',
        });
      else {
        res.json({ type: 'failure', result: 'An Error Occured' });
      }
    } else {
      res.json({ type: 'failure', result: 'Fill out form correctly' });
    }
  } catch (error) {
    console.log(error);
    res
...
exports.GraphData = async (req, res) => {
  try {
    ...
    const userCounts = labels.map(
      (month) =>
        users.filter(
          (user) => user.createdAt.getMonth() === labels.indexOf(month)
        ).length
    );
    ...
    const postCounts = labels.map(
      (month) =>
        posts.filter(
          (post) => post.createdAt.getMonth() === labels.indexOf(month)
        ).length
    );
...
exports.NoticeComplete = async (req, res) => {
  try {
    ...
    if (complete) {
      const notices = await ContactModal.find({ completed: false }).sort({
        $natural: -1,
      });
